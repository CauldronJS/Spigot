...
function resolveRequireRoot(request, currentDir) {
  ...
  if (isRelative) {
    ...
    while (
      path.normalize(currentAccessAt) !==
      path.normalize(path.resolve(currentAccessAt, '..'))
    ) {
...
function tryGetJavaObject(request) {
  try {
    ...
    if (Object.keys(fallbackPackage).length === 0)
      console.error(`Could not locate Java package ${request}`);
